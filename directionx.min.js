/*
 ...
 b=c[2].transferToImageBitmap(),e(b)}}J?(b=J.createKernel(g.shader||function(a){var b=4*(this.thread.x+this.constants.w*(this.constants.h-this.thread.y));this.color(a[b]/256,a[b+1]/256,a[b+2]/256,a[b+3]/256)}).setConstants({w:b[0],h:b[1]}).setOutput(b).setGraphical(!0),b(l.ui.data),createImageBitmap(b.canvas).then(e)):(b=g.pixelfn,createImageBitmap(b?b(l.ui):l.ui).then(e))},p=function(a){if(f.length){A=!0;F(G);m.run("start");a=Math.min(Math.max(0,a),f.length-1);f[a].ib?f[a].st!=f.shaderTime?w(f[a]):
 I(f[a].ib):(u.clearRect(0,0,c[1].width,c[1].height),x.virID=a,x.src=k.dir+f[a].s);n=a;var b,l=0;for(b=a-1;l<k.irb&&0<=b;b--)if(!f[b].ib)if(f[b].st!=f.shaderTime)w(f[b]);else{var e=l,g=k.dir+f[b].s;y[e].virID=b;y[e].src=g;l++}for(b=a+1;l<k.itb&&b<f.length;b++)f[b].ib||(f[b].st!=f.shaderTime?w(f[b]):(a=l,e=k.dir+f[b].s,y[a].virID=b,y[a].src=e,l++))}},K=function(a,b){if(a)for(var c in b)a.hasOwnProperty(c)&&(a[c]=b[c])},r=function(a){return Array(a).fill(1).map(function(a,c){var b=new Image;b.unique=
 c;b.virID=-1;b.addEventListener("load",w.bind(null,b));return b})},L=function(a){var b=Date.now();a=("string"==typeof a?a.split(" "):a).map(function(a,c,e){return{s:a,st:b,d:c?c==e.length-1?1:0:-1}});a.shaderTime=b;return a},O=function(){try{jQuery.fn.direction=function(a,b){return this.each(function(){b.anchor=$(this);d(a,b)})}}catch(a){console.log(a)}},q=document,B=q.body,t=q.documentElement,z=parseInt,F=window.requestAnimationFrame,P=g.overwrite||0,M=g.anchor||B,f=L(h),A=!0,C=-1,n=-1,c={0:g.disableSpin?
 null:q.createElement("canvas"),1:q.createElement("canvas"),2:new OffscreenCanvas(640,480)},e=c[0]?{ctx:c[0].getContext("2d"),clr:g.color||"#373737",str:Date.now(),lne:g.lines||16,rte:g.rate||1E3/30,dia:g.diameter||250,lbk:g.loaderback||"#FFF",dat:Date.now(),sav:[]}:null,k={dir:g.dir||"",irb:g.imgprebuffer||5,itb:g.imgpostbuffer||5,bck:g.back||"#FFF",sz:g.size||0,scl:0},y=r(k.itb+k.irb),x=r(1)[0],D=!0,u=c[1].getContext("2d"),v=c[2].getContext("2d"),m={run:function(a){for(var b=0;b<m[a].length;b++)m[a][b]()},
