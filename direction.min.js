/*
 ...
 void 0===a.y)a.y=0;if(null===a.x||void 0===a.x)a.x=0}else a={x:0,y:a};if(null===d||void 0===d||0>=d)d=400;0>a.y&&(a.y=window.innerHeight||w.clientHeight||z.clientHeight);0>a.x&&(a.x=window.innerWidth||w.clientWidth||z.clientWidth);var c={x:void 0!==window.pageXOffset?a.x-window.pageXOffset:a.x-w.scrollLeft,y:void 0!==window.pageYOffset?a.y-window.pageYOffset:a.y-w.scrollTop};if(c=={x:0,y:0})return c;var b=function(a,d,c){window.scrollBy(Math.floor(a.x)/d,Math.floor(a.y)/d);c+1<5*d&&(B=window.setTimeout(b,
 5,a,d,c+1))};B=window.setTimeout(b,5,c,Math.floor(d/5),0);return c};p=function(){f[this.virID].ld=!0};var n=function(a,d){A=!0;window.setTimeout(F,l.rate,E);h.run("start");0>d&&(d=0);d>=f.length&&(d=f.length-1);if(!(0>d)){f[d].ld||x.clearRect(0,0,e[1].width,e[1].height);a.virID=d;a.src=k.dir+f[d].s;m=d;var c=0,b;for(b=d-1;b>d-k.irb-1&&0<=b;b--)f[b].ld||(u[c].virID=b,u[c].src=k.dir+f[b].s,c++);c=0;for(b=d+1;b<k.itb+d+1&&b<f.length;b++)f[b].ld||(t[c].virID=b,t[c].src=k.dir+f[b].s,c++)}},H=function(a,
 ...
 "170px";e[0].style.zIndex=0;e[0].style.position="absolute";q.appendChild(e[1]);window.setTimeout(F,l.rate,E);g=new Image;g.virID=-1;g.addEventListener("load",function(){f[g.virID].ld?x.clearRect(0,0,e[1].width,e[1].height):f[g.virID].ld=!0;h.run("slidn");if(k.sz)var a=[k.sz.w,k.sz.h];else switch(k.scl){case 1:a=e[1].width/g.width;a=[e[1].width,g.height*a];break;case 2:a=e[1].height/g.height;a=[g.width*a,e[1].height];break;default:a=k.scl?[e[1].width,e[1].height]:[g.width,g.height]}e[1].width=a[0];
 e[1].height=e[0].height=a[1];x.drawImage.apply(x,[g,0,0].concat(a));A=0;D&&G();h.run("slidd")},!1);for(b=0;b<f.length;b++)f[b].d=b?b==f.length-1?1:0:-1,f[b].ld=!1;for(b=0;b<k.irb;b++)u.push(new Image),u[b].virID=-1,u[b].addEventListener("load",p,!1);for(b=0;b<k.itb;b++)t.push(new Image),t[b].virID=-1,t[b].addEventListener("load",p,!1);n(g,k.startpage||y);e[1].height=480;e[1].width=640;e[1].background=k.back;e[1].style.zIndex=1;e[1].style.position="relative";q.appendChild(e[1])};